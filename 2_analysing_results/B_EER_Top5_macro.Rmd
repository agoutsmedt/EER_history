...
In the section [EER Bibliographic Information] of this appendix, we compiled bibliographic information related to all articles published in the European Economic Review (EER). The goal of this section is to provide general information about the journal, to contextualize the following network analysis. Users can explore information about the most cited references, the most productive authors in the journal, or the type of collaboration between US and European economists in the journal.

In the section [Dynamics Networks], we propose a network analysis of all macro articles, identified by JEL codes, published in the top five economics journals and the EER. In this section, we explore the structure of macroeconomics publications in these journals by identifying clusters of articles that tackle similar subjects using their bibliography. From these clusters, we identify clusters that are more European-oriented (research published in the EER and/or written by European authors) from clusters that are more US-oriented.

Finally, in section [A Look at Dynamic Communities] users can explore information about the individual clusters identified in the previous section. For example, we produced a variety of tables to explore the most cited references of the cluster or its most distinctive words/institutions compared to the overall corpus. In addition, users can also find some intra-clusters information about how the cluster evolved overtime, or the differences in the clusters between European/US authors and/or venues of publications.

```{r, include=FALSE, set.seed(333)}
knitr::opts_knit$set(root.dir = "D:/Dropbox/8-Projets Quanti/1-R_Projects/Home/EER_history")
setwd("D:/Dropbox/8-Projets Quanti/1-R_Projects/Home/EER_history")
...
## All articles in the EER

In the following table, all articles published in the EER used for this section are available to explore:

```{r, message=FALSE, warning=FALSE, error=FALSE, results=TRUE, cache=FALSE}

datatable(Corpus_all_EER[,.(Nom,Annee_Bibliographique,Titre,Revue,EU_US_collab)], rownames = FALSE, class = 'cell-border stripe')
...
institutions_unique <- Institutions_all_EER[,head(.SD, 1),.(ID_Art,Institution)]
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=EU_US_collab, color=EU_US_collab)) +
  ...
  scale_y_continuous("Share of Papers Authored by Affiliations", limits = c(0, 0.8), breaks = seq(0,.8,0.2)) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  ...
  coord_cartesian(xlim = c(NA, 2010))
...
The three following graph informs us about the share of 7 economics journals in the references of (1) all articles in social sciences, (2) all articles in economics, and (3) all articles in macroeconomics. The complete list of journal is the *European Economic Review* (EER), *The Scandinavian Journal of Economics* (Scandinavian), *The Economic Journal*, the *Review of Economic Studies, Economica*, *Oxford Economic Papers*, and the *Weltwirtschaftliches Archiv*. We use these journals as benchmark to measure the changing relative importance of the EER in the landscape of economics when compared to other European and US journals.
...
journal_name <- tribble(
  ~Code_Revue, ~Journal_label,
  "5200", "EER",
  "14501", "Scandinavian",
  "4713", "Economics Journal",
  "13992", "Review of Economic Studies",
  "4719", "Economica",
  "12488", "Oxford Economic Papers",
  "16273", "Weltwirtschaftliches Archiv")
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  scale_y_continuous("Share of Journals in Social Sciences Refs", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2010)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_color_brewer(palette = "Dark2", guide = "none")
ggsave(here(eer_data,"pictures","Graphs","EER_importance.png"), width=30, height=30, units = "cm", scaling = 1.7)
...
ggplot(count[Annee_Bibliographique>1970  & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of Journals in All Economics Refs", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2010)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
...
ggsave(here(eer_data,"pictures","Graphs","EER_importance_econ.png"), width=30, height=30, units = "cm", scaling = 1.7)
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of Journals in All Macroeconomics Refs", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2010)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_color_brewer(palette = "Dark2", guide = "none")
ggsave(here(eer_data,"pictures","Graphs","EER_importance_macro.png"), width=30, height=30, units = "cm", scaling = 1.7)
...
EER importance in all articles in macroeconomic:
...
In this section we explore specific informations related to developments in the article
...
g2 = data.table(
  ...
  label_special = c("Lucas (1972)","Barro (1976)","Sargent and Wallace (1975)","Kydland and Prescott (1982)", "Lucas (1973)")
)
...
ggplot(g1_citation, aes(x=Annee_Bibliographique , y=share, color=color)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of Articles in Corpus References", labels = scales::percent_format(scale=1), limits=c(0,NA)) +
  coord_cartesian(xlim = c(NA, 2010)) +
  scale_color_discrete(guide = "none") +
  ggrepel::geom_label_repel(aes(label = label_special), data = g1_citation[Annee_Bibliographique==2000, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g1_citations.png"), width=40, height=20, units = "cm", scaling = 1.5)
...
ggplot(obs_th_comparison[!is.na(y)], aes(x=Annee_Bibliographique , y=-1*y, color=color, group=color)) +
  geom_point(alpha=1) +
  # geom_line() +
  geom_hline(yintercept = 0, size = 1, alpha = 1, linetype = "dashed") +
  ...
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  ggrepel::geom_label_repel(aes(label = label_special), data = obs_th_comparison[Annee_Bibliographique==1999, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3) +
  coord_cartesian(xlim = c(NA, 2005), ylim = c(-0.1,NA)) +
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g1_over_citations.png"), width=40, height=20, units = "cm", scaling = 1.5)
...
ggplot(g2_citation[Annee_Bibliographique<=1995], aes(x=Annee_Bibliographique , y=share, color=color)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of Articles in Corpus Citing References", labels = scales::percent_format(scale=1), limits=c(0,NA)) +
  coord_cartesian(xlim = c(NA, 2000)) +
  scale_color_discrete(guide = "none") +
  ggrepel::geom_label_repel(aes(label = label_special), data = g2_citation[Annee_Bibliographique==1995, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g2_citations.png"), width=40, height=20, units = "cm", scaling = 1.5)
...
ggplot(obs_th_comparison[!is.na(y) & Annee_Bibliographique<=1992], aes(x=Annee_Bibliographique , y=-1*y, color=color, group=color)) +
  ...
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  ggrepel::geom_label_repel(aes(label = label_special), data = obs_th_comparison[Annee_Bibliographique==1992, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3) +
  coord_cartesian(xlim = c(NA, 1995)) +
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g2_over_citations.png"), width=40, height=20, units = "cm", scaling = 1.5)
...
We measured the relative importance of particular articles in the corpus by computing the share of articles in the corpus citing these particular references:
...
We measured the over and under citations by European and American economists of particular references by calcuting the log ratio of the share of european and american economists citing particular articles on the share of overall citations by european and american economists in the corpus with right aligned moving average on 8 years:

${\scriptstyle \text{EU/US Over-citations}=\log(\frac{\text{Share of American Citing Reference}}{\text{Overall Share of American Citing}})}$
...
for (com in com_to_inspect) {
  ...
  # europeans_authors_diff <- log(log(share_europeans_authors_cluster/share_US_authors_cluster)/log((share_europeans_authors/share_US_authors)))
...
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_colored_odds.png"), width=40, height=20, units = "cm", scaling = 1.5)
...
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_colored.png"), width=40, height=20, units = "cm", scaling = 1.5)
