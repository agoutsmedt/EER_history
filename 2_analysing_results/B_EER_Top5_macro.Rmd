...
ggplot(mean_jel[V1!=0 & Annee_Bibliographique!="2016"], aes(x=as.numeric(Annee_Bibliographique), y=V1)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.8,color="black")+
...
ggsave(here(eer_data,"pictures","Graphs","mean_jel.png"), width=35, height=20, units = "cm")
...
## Collaborations between Europeans and Americans in the EER
...
ggsave(here(eer_data,"pictures","Graphs","Collab_strict.png"), width=35, height=20, units = "cm")


```

The following graph informs us about the collaborations between American and European authors in the EER:
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  scale_x_continuous("Years") +
  scale_y_continuous("Share of EER in Social Sciences Refs", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2006)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA))
ggsave(here(eer_data,"pictures","Graphs","EER_importance.png"), width=35, height=20, units = "cm")
...
ggplot(count[Annee_Bibliographique>1970  & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of EER in All Economics Refs", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2006)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA))
ggsave(here(eer_data,"pictures","Graphs","EER_importance_econ.png"), width=35, height=20, units = "cm")
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA))
ggsave(here(eer_data,"pictures","Graphs","EER_importance_macro.png"), width=35, height=20, units = "cm")
...
For all macroeconomics articles published in the EER and in the Top 5, we build the networks with 8-year overlapping windows.

We used Leiden detection algorithm that optimize the modularity on each network to identify groups of articles that are similar to each others and dissimilar to the rest of the network.Because networks have a lot of overlaps, many clusters between two periods are composed of the same articles. To identify these clusters that are very similar between two time windows, we considered that *(i)* if at least 55% of the articles in a community of the first time window where in the same cluster in the second time window, and that *(ii)* if the cluster was also composed by at least 55% of articles of the first time window, *then* it is the same cluster.

## Alluvial

The following plot is a way to represent these networks and communities as an alluvial that tracks how nodes that exists across multiple overlapping windows change or stay in the same communities:
...
## European/American identity of clusters
...
for (com in com_to_inspect) {
  ...
  europeans_authors_diff <- log(share_europeans_authors_cluster/share_US_authors_cluster)-log(share_europeans_authors/share_US_authors)
  EER_articles_diff <- share_EER_articles_cluster-share_EER_articles
  EER_articles_diff <- log((share_EER_articles_cluster/(1-share_EER_articles_cluster)))-log((share_EER_articles/(1-share_EER_articles)))
...
ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff, y = EER_articles_diff)) +
  ...
  scico::scale_color_scico(palette = "roma", breaks = c(-1, 1), labels = c("Less European", "More European")) +
  labs(title = "",
       color = "",
       x = "US Only (left) vs. European Only (right)",
       y = "Top 5 (down) vs. EER (up)") + 
