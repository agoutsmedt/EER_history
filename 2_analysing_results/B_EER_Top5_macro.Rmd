...
datatable(Corpus_all_EER, rownames = FALSE, class = 'cell-border stripe')
...
```{r, message=FALSE, warning=FALSE, error=FALSE, results=TRUE, cache=FALSE}
Authors_EER[,.N,Nom_ISI][order(-N)] %>% top_n(20)
```

## Most Productive Institutions and Countries

```{r, message=FALSE, warning=FALSE, error=FALSE, results=TRUE, cache=FALSE}
Institutions_all_EER[,.N,Pays][order(-N)] %>% top_n(20)
Institutions_all_EER[,.N,Institution][order(-N)] %>% top_n(20)
```
...
kable(Refs_all_EER[,.N, Revue][order(-N)]%>% top_n(50)) %>% kable_styling(bootstrap_options = c("striped", "condensed", full_width = F))

```


## EER Popularity overall
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, error=FALSE, results=TRUE, out.width='100%'}
institutions <- institutions_unique
...
alluv_dt_as_nodes[,Id:=as.character(Id)]
...
most_influential_nodes <- merge(most_influential_nodes,Authors[Ordre==1,.(Nom,ID_Art)],by.x="Id",by.y="ID_Art", all.X=TRUE)
# Label column
most_influential_nodes <- most_influential_nodes[, name_short:=  gsub("-.*","",Nom)]
most_influential_nodes$name_short <- toupper(most_influential_nodes$name_short)
most_influential_nodes <- most_influential_nodes[,Label:=paste0(name_short,",",Annee_Bibliographique)]
most_influential_nodes[, c("name_short"):=NULL]

com_to_inspect <- alluv_dt_as_nodes[share_leiden_max>= min_leiden_max & n_years>=min_n_years, .N, Label_com][order(-N)]$Label_com
...
diff_stat_euro_com <- merge(diff_stat_euro_com, new_Id_com_size, by="new_Id_com", all.x = TRUE)
...
ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff, y = EER_articles_diff)) +
  ...
  theme_minimal() 
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_bw_test.png"), width=35, height=20, units = "cm")
...
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_colored_test.png"), width=35, height=20, units = "cm")
...
for (com in com_to_inspect) {
  ...
  alluv_com  <- alluv_dt_as_nodes[Label_com %like% paste0(com)]
  
  most_influential_nodes_com  <- most_influential_nodes[Label_com %like% paste0(com)][order(-weighted_size)]
  ...
  most_productive_countries <- merge(alluv_com[,.(Id)], institutions[,.(ID_Art, Pays)], by.x= "Id", by.y= "ID_Art")
  most_productive_countries <- most_productive_countries[,.N, Pays]
  most_productive_countries <- most_productive_countries[order(-N)]

  
  refs_td_idf <- merge(most_influential_nodes_com[,.(Id, Revue)], Refs, by="Id")
  refs_td_idf <- merge(refs_td_idf, institutions_unique[,head(.SD, 1),.(ID_Art,EU_US_collab)][,.(ID_Art,EU_US_collab)],by="ID_Art",all.x = TRUE)
  ...
  refs_td_idf_collab <- merge(most_influential_nodes_com[,.(Id, Revue)], Refs, by="Id")
  refs_td_idf_collab <- merge(refs_td_idf_collab, institutions_unique[,head(.SD, 1),.(ID_Art,EU_US_collab)][,.(ID_Art,EU_US_collab)],by="ID_Art",all.x = TRUE)
 ...
 #  
 #  word_ratios <- merge(word_ratios,refs_td_idf[,.N,.(ItemID_Ref,Label_Target)][order(-N),head(.SD,1),.(ItemID_Ref)],by="ItemID_Ref",all.x = TRUE)
 #  EER_ratio_plot <- word_ratios %>%
 #    group_by(logratio < 0) %>%
 #    slice_max(abs(logratio), n = 20) %>% 
 #    ungroup() %>%
 #    mutate(Label_Target = reorder(Label_Target, logratio)) %>%
 #    ggplot(aes(Label_Target, logratio, fill = logratio < 0)) +
 #    geom_col(show.legend = FALSE) +
 #    coord_flip() +
 #    ylab("log odds ratio (EER/TOP5)") +
 #    scale_fill_discrete(name = "", labels = c("EER", "TOP5")) +
 #    theme_minimal()
 #    
 #  word_ratios <- refs_td_idf %>%
 ...
 #  word_ratios <- merge(word_ratios,refs_td_idf[,.N,.(ItemID_Ref,Label_Target)][order(-N),head(.SD,1),.(ItemID_Ref)],by="ItemID_Ref",all.x = TRUE)
 # 
 # Europeans_ratio_plot <- word_ratios %>%
 #    group_by(logratio < 0) %>%
 #    slice_max(abs(logratio), n = 20) %>% 
 #    ungroup() %>%
 #    mutate(Label_Target = reorder(Label_Target, logratio)) %>%
 ...
 #    geom_col(show.legend = FALSE) +
 #    coord_flip() +
 #    ylab("log odds ratio (Europe Only /USA Only)") +
 #    scale_fill_discrete(name = "", labels = c("Europe Only", "USA Only")) +
 #    theme_minimal()
 #  }
  ...
  cat("\n\n")
  print(kable(most_influential_nodes_com[EU_US_collab=="Europe Only",.(Label, Titre, Revue, EU_US_collab, weighted_size)][, head(.SD, 8)]) %>% 
          kable_styling(bootstrap_options =     c("striped", "condensed", full_width = F)))
  cat("\n\n")
  
  cat("\n\n")
  print(kable(most_influential_nodes_com[Revue %like% "EUROPEAN%",.(Label, Titre, Revue, EU_US_collab, weighted_size)][, head(.SD, 8)]) %>% 
          kable_styling(bootstrap_options =     c("striped", "condensed", full_width = F)))
  cat("\n\n")
  ...
  cat("The most common refs (new_id2 without ItemID_Ref):")
  cat("\n\n")
  print(kable(most_influential_noidrefs_com[,.(Label_Target, Revue_Abbrege, share)][, head(.SD, 20)]) %>% 
          kable_styling(bootstrap_options =     c("striped", "condensed", full_width = F)))
  cat("\n\n")
  

  cat("###","Authors and Institutions:", "\n")
  ...
  cat("\n\n")
    ...
    cat("The most productive Countries of the community:")
  cat("\n\n")
  print(kable(most_productive_countries[,.(Pays, N)][, head(.SD, 10)]) %>% 
          kable_styling(bootstrap_options =     c("striped", "condensed", full_width = F)))
  cat("\n\n")
  
  cat("###","Distinctiveness EU/US:", "\n")
  # cat("\n\n")
  # if(refs_td_idf[,.N,EER_pub_bin][,.N]==2 & refs_td_idf[,.N,EU_US_collab][EU_US_collab=="USA Only" | EU_US_collab=="Europe Only",.N]==2){
  # plot(EER_ratio_plot)
  #   }
  ...
  cat("Most distinctive refs EER:")
  cat("\n\n")
  print(kable(refs_td_idf[order(-tf_idf)][EER_pub_bin=="EER", head(.SD,1),ItemID_Ref][, head(.SD, 20)][,.(Label_Target,Titre, Revue_Abbrege, share, tf_idf)]) %>% 
          kable_styling(bootstrap_options =     c("striped", "condensed", full_width = F)))
  cat("\n\n")
  
  cat("Most distinctive refs TOP5:")
  cat("\n\n")
  ...
  cat("Temporal Differences first 5 years vs last 5 years:")
  cat("\n\n")
