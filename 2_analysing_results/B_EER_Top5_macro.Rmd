...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_x_continuous("Years") +
  scale_y_continuous("Share of EER in All Macroeconomics Refs", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2010)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
...
For each clusters, we identified the US or European oriented nature of its publications and authors. A first measure we used is the over/under representation of European/US authors in the cluster. For each cluster, we subtracted the relative share of European authors to US authors in the cluster, to the relative share of European authors to US authors on the same time window of the cluster leading us to the first index:
...
ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff, y = EER_articles_diff)) +
  ...
  ggrepel::geom_text_repel(data = . %>% filter(sum_diff > 0.01 | sum_diff < -0.012), 
                           aes(label = as.character(Label_com)), alpha = 1, hjust = 0, size=1.7) +
  ...
  theme(legend.position = "bottom") +
  guides(size = "none") 
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_colored.png"), width=30, height=20, units = "cm", scaling = 1.7)
