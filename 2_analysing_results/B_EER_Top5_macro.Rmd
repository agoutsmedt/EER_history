...
ggsave(here(eer_data,"pictures","Graphs","EER_affiliations.png"), width=35, height=20, units = "cm")
...
ggplot(main_boards, aes(Year, after_stat(count), group = fct_reorder(Pays, rank), fill = color)) +
  ...
  theme(legend.position = "bottom")  +
  guides(fill = guide_legend(nrow = 4))
ggsave(here(eer_data,"pictures","Graphs","Board_affiliations.png"), width=35, height=20, units = "cm")
...
ggsave(here(eer_data,"pictures","Graphs","EER_collab_stack_density.png"), width=35, height=20, units = "cm")
...
# all_art <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_art.parquet"))


# citations_journals_econ
...
ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff, y = EER_articles_diff)) +
  ...
  scico::scale_color_scico(palette = "roma", breaks = c(-2.2, 1), labels = c("Less European", "More European")) +
  labs(title = "",
       color = "",
       x = "US Only (left) vs. European Only (right)",
       y = "Top 5 (down) vs. EER (up)") + 
