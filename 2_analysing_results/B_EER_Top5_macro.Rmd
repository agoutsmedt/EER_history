...
journal_name <- tribble(
  ...
  "4713", "Economics Journal", "dashed", "#a0a0a0",
...
for (com in com_to_inspect) {
  ...
  share_europeans_authors_sd <- alluv_dt_as_nodes[Window>=min(alluv_com$Window) & Window<=max(alluv_com$Window) & (EU_US_collab=="USA Only" | EU_US_collab=="Europe Only")] %>% 
  ...
  share_europeans_authors_cluster_sd <- alluv_com[EU_US_collab=="USA Only" | EU_US_collab=="Europe Only"] %>%
  ...
  europeans_authors_sd <- (share_europeans_authors_cluster_noodds-share_europeans_authors_noodds) / sqrt((share_europeans_authors_cluster_sd^2/share_europeans_authors_cluster_sd_n) + (share_europeans_authors_sd^2/share_europeans_authors_sd_n))
  europeans_authors_hdi <- (log10(share_europeans_authors_cluster_noodds)-log10(share_europeans_authors_hdi_min)) / log10((share_europeans_authors_hdi_max)-log10(share_europeans_authors_hdi_min))
  ...
  EER_articles_sd <- (share_EER_articles_cluster-share_EER_articles) / sqrt((share_EER_articles_cluster_sd^2/share_EER_articles_sd_cluster_sd_n) + (share_EER_articles_sd^2/share_EER_articles_sd_n))
  EER_articles_hdi <- (log10(share_EER_articles_cluster)-log10(share_EER_articles_hdi_min)) / (log10(share_EER_articles_hdi_max)-log10(share_EER_articles_hdi_min))
...
plot_bw <- ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff, y = EER_articles_diff)) +
  ...
  ggrepel::geom_text_repel(data = . %>% filter((sum_diff > 0.01 | sum_diff < -0.012) & new_Id_com_size>51),
                           ...
                           box.padding=0.15,
                           max.overlaps = 4) +
  scico::scale_color_scico(palette = "grayC", breaks = c(-1, 0.40), labels = c("Less European", "More European"), begin = 0.3) +
  labs(title = NULL,
       color = NULL,
       x = "US Only (left) vs. European Only (right)",
       y = "Top 5 (down) vs. EER (up)") + 
