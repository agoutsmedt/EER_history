...
ggplot(mean_jel[V1!=0 & Annee_Bibliographique!="2016"], aes(x=as.numeric(Annee_Bibliographique), y=V1)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.75,color="black", linetype = "dashed")+
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=EU_US_collab, color=EU_US_collab)) +
  ...
  scale_y_continuous("Share of Papers Authored by Affiliations", limits = c(0, 0.8), breaks = seq(0,.8,0.2), labels = scales::percent_format(scale=1)) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  scale_y_continuous("Share of Journals in Social Sciences References", labels = scales::percent_format(scale=1)) +
...
ggplot(count[Annee_Bibliographique>1970  & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  geom_point(alpha=0.1) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_y_continuous("Share of Journals in All Economics References", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2010)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002], aes(x=Annee_Bibliographique , y=share,color=Journal_label)) +
  ...
  geom_point(alpha=0.1) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_y_continuous("Share of Journals in All Macroeconomics References", labels = scales::percent_format(scale=1)) +
  coord_cartesian(ylim = c(0,NA), xlim = c(NA, 2010)) +
  scale_color_discrete(guide = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA, size=3) +
  theme_minimal() +
...
ggplot(obs_th_comparison[!is.na(y)], aes(x=Annee_Bibliographique , y=-1*y, color=color, group=color)) +
  ...
  labs(title = NULL,
       color = NULL,
       x = NULL) + 
  scale_y_continuous("Over-citations Americans (bottom) vs Europeans (top)", labels = scales::percent_format(scale=1)) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  ...
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g1_over_citations.png"), width=40, height=20, units = "cm", scaling = 1.7)
...
ggplot(g2_citation[Annee_Bibliographique<=1995], aes(x=Annee_Bibliographique , y=share, color=color)) +
  ...
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g2_citations.png"), width=40, height=20, units = "cm", scaling = 1.7)
...
ggplot(obs_th_comparison[!is.na(y) & Annee_Bibliographique<=1992], aes(x=Annee_Bibliographique , y=-1*y, color=color, group=color)) +
  ...
  scale_colour_identity()
ggsave(here(eer_data,"pictures","Graphs","g2_over_citations.png"), width=40, height=20, units = "cm", scaling = 1.7)
...
We measured the relative importance of particular articles in the corpus by computing the share of articles in the corpus citing these particular references (7 years moving average):
...
${\displaystyle\text{EU/US Over-citations}= -1 \times\log(\frac{\text{Share of Specific Citations from American Articles}}{\text{Share of Citations from American Articles}})}$
