...
```{r, message=FALSE, warning=FALSE, error=FALSE, results=TRUE, cache=FALSE}
table_europeanisation <- merge(alluv_dt_as_nodes[,.(Id, new_Id_com, Label_com)], Institution, by.x = "Id", by.y = "ID_Art", all.x = TRUE, allow.cartesian = TRUE)
table_europeanisation <- table_europeanisation %>%
  mutate(Pays = ifelse(Pays %in% c("SCOTLAND", "ENGLAND", "WALES", "UNITED KINGDOM"), "UK", Pays))
...
table_countries <- table_europeanisation[,.N,.(Label_com ,Pays)][,perc:=100*N/sum(N),.(Label_com )]
table_countries[order(-perc), rank:=sequence(.N),Label_com ]
table_countries[, Pays_label := paste0(Pays, " (", round(perc,2) , "%)" )]

table_countries <- dcast(table_countries[rank<=10], rank ~Label_com, value.var = "Pays_label")
