...
If we look at the distribution of topics only for highest gamma, using the logratio for affiliations and journals:
...
ggplot(topic_diff_summary, aes(x = logratio_affiliation, y = logratio_journal)) +
  ...
  geom_point(aes(group = factor(Com_name),
                 ...
                 fill = com_color,
                 size = topic_prevalence), alpha = 0.8, show.legend = FALSE) +
  geom_text_repel(data= topic_diff_summary[topic_prevalence>0.01], aes(label = topic_label, color="black"), size = 2, alpha = 1, hjust = 0, max.overlaps = 4, min.segment.length = 0.1, 
                  segment.linetype=	"dashed", 
                  segment.size = 0.3,) +
  labs(title = NULL,
       x = "US Only (left) vs. European Only (right)",
...
ggsave(here(eer_data,"pictures","Graphs","logratio_diff_plot_new_bw.png"), width=40, height=20, units = "cm", scaling = 1.5)
