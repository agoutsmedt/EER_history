---
title: "Bibliographic information about the EER and details on the bibliographic coupling clusters and topic modelling"
...
For each year, we calculated the share of articles published in the EER that matched macroeconomics JEL codes to get an idea of the changing importance of macroeconomics in the journal:
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=EU_US_collab, color=EU_US_collab)) +
  ...
  scale_y_continuous("Share of papers nelonging to a pattern of collaboration", limits = c(0, NA), breaks = seq(0,.8,0.2), labels = scales::label_percent()) +
  theme_minimal() +
...
ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, group=EU_US_collab, linetype =EU_US_collab, color=EU_US_collab)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.4) +
  # geom_point(alpha=0.2) +
  scale_x_continuous(NULL) +
  scale_y_continuous("Share of papers nelonging to a pattern of collaboration", limits = c(0, NA), breaks = seq(0,.8,0.2), labels = scales::label_percent()) +
  theme_minimal() +
  theme(axis.title = element_text(size = 8), plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_color_manual(values = c("#a0a0a0", "black", "#a0a0a0", "black"),guide = FALSE) +
...
#                   label="Smoothed using local polynomial regression",
#                   color="Darkgrey")
ggsave(here(eer_data,"pictures","Graphs","Collab_strict_bw.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
intra_eu_collab <- copy(institutions_unique[EU_US_collab=="Europe Only"])

intra_eu_collab <- intra_eu_collab[,head(.SD, 1),.(ID_Art,Pays)]
intra_eu_collab[, n_diversity_collab:=.N ,.(ID_Art)]
intra_eu_collab[, intra_eu_collab_bin:=0]
intra_eu_collab[n_diversity_collab>1, intra_eu_collab_bin:=1]

intra_eu_collab[,n_diversity_collab:=as.character(n_diversity_collab)]
intra_eu_collab[n_diversity_collab>=4, n_diversity_collab:="4+"]
intra_eu_collab[,n_diversity_collab:=paste0(n_diversity_collab," countries")]
intra_eu_collab[n_diversity_collab=="1 countries", n_diversity_collab:="1 country"]

count_year <- intra_eu_collab[,.N,.(ID_Art,n_diversity_collab,Annee_Bibliographique)][order(N)]
count_year <- count_year %>% group_by(Annee_Bibliographique, n_diversity_collab) %>% summarise(n = n()) %>%  mutate(freq = n / sum(n)) %>% as.data.table()
count_year <- complete(count_year, Annee_Bibliographique, n_diversity_collab) %>% as.data.table
count_year[is.na(freq),freq:=0]

ggplot(count_year, aes(x=as.numeric(Annee_Bibliographique), y=freq, linetype=n_diversity_collab, color=n_diversity_collab)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.3) +
  # geom_point(alpha=0.2) +
  scale_x_continuous(NULL) +
  scale_y_continuous("Share of articles with x countries in affiliations", limits = c(0, NA), labels = scales::label_percent()) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_color_manual(values = c("#a0a0a0", "black", "#a0a0a0", "black", "#a0a0a0"),guide = FALSE) +
  guides(linetype = FALSE) +
  ggrepel::geom_label_repel(aes(label = n_diversity_collab), data = count_year[Annee_Bibliographique==2002], nudge_x = 1, segment.color = NA) +
  coord_cartesian(xlim = c(NA, 2008))
# ggplot2::annotate("text", x = max(2008), y = min(count_year$freq), hjust=0.95, vjust=-1,
#                   label="Smoothed using local polynomial regression",
#                   color="Darkgrey")
ggsave(here(eer_data,"pictures","Graphs","Intra_EU_collab_n_country.png"), width=30, height=20, units = "cm", scaling = 1.7)


count_year <- intra_eu_collab[,.N,.(ID_Art,intra_eu_collab_bin,Annee_Bibliographique)][order(N)]
count_year <- count_year %>% group_by(Annee_Bibliographique, intra_eu_collab_bin) %>% summarise(n = n()) %>%  mutate(freq = n / sum(n)) %>% as.data.table()
count_year <- complete(count_year, Annee_Bibliographique, intra_eu_collab_bin) %>% as.data.table
count_year[is.na(freq),freq:=0]

ggplot(count_year[intra_eu_collab_bin==1], aes(x=as.numeric(Annee_Bibliographique), y=freq)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.4) +
  # geom_point(alpha=0.2) +
  scale_x_continuous(NULL) +
  scale_y_continuous("Share of articles written by Europeans authors from different countries", limits = c(0, NA), labels = scales::label_percent()) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  # scale_color_manual(values = c(rev(brewer.pal(n = 7, name = "Dark2"))),guide = FALSE) +
  # scale_color_discrete( +
...
#                   label="Smoothed using local polynomial regression",
#                   color="Darkgrey")
ggsave(here(eer_data,"pictures","Graphs","Intra_EU_collab_bin.png"), width=30, height=20, units = "cm", scaling = 1.7)

count_year_all <- merge(count_year, share_EU_US_collab_save[,sum(n), Annee_Bibliographique], by = "Annee_Bibliographique", all.x = TRUE)
count_year_all[,freq := n/V1]
count_year_all[is.na(freq),freq:=0]
setnames(count_year_all, "intra_eu_collab_bin", "EU_US_collab")
count_year_all[,EU_US_collab:=as.character(EU_US_collab)]
count_year_all[EU_US_collab==1, EU_US_collab:="Intra EU Collab"]
count_year_all <- count_year_all[EU_US_collab!=0]
count_year_all <- rbind(count_year_all[,.(Annee_Bibliographique, EU_US_collab, freq)], share_EU_US_collab_save[EU_US_collab!="Neither",.(Annee_Bibliographique, EU_US_collab, freq)])
count_year_all[EU_US_collab=="Collaboration", EU_US_collab:="EU-US collaboration"]
count_year_all[EU_US_collab=="Europe Only", EU_US_collab:="European authors"]
count_year_all[EU_US_collab=="USA Only", EU_US_collab:="US authors"]
count_year_all[EU_US_collab=="Intra EU Collab", EU_US_collab:="Intra-EU Collaboration*"]

ggplot(count_year_all, aes(x=as.numeric(Annee_Bibliographique), y=freq, linetype=EU_US_collab, color=EU_US_collab)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.4) +
  # geom_point(alpha=0.2) +
  scale_x_continuous(NULL) +
  scale_y_continuous("Share of articles", limits = c(0, NA), breaks = seq(0,1,0.2), labels = scales::label_percent()) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_color_manual(values = c("#818181", "black", "#c2c2c2", "black")) +
  guides(linetype = FALSE, color= FALSE) +
  ggrepel::geom_label_repel(aes(label = EU_US_collab), data = count_year_all[Annee_Bibliographique==2001], nudge_x = 2, nudge_y = 0.01, segment.color = NA, force = 2, force_pull = 10, size = 3) +
  coord_cartesian(xlim = c(NA, 2008)) +
  theme(axis.title = element_text(size = 8))  
# ggplot2::annotate("text", x = max(2008), y = min(count_year$freq), hjust=0.95, vjust=-1,
#                   label="Smoothed using local polynomial regression",
#                   color="Darkgrey")
ggsave(here(eer_data,"pictures","Graphs","Intra_vs_US_collab.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
The following graphs informs us about the occurrences of particular institutions and countries in our corpus:
...
levels <- tibble("Pays" = as.factor(main_institutions[,.N,Pays][order(Pays)]$Pays)) %>%
  filter(Pays != "OTHER") %>%
  rbind("OTHER") %>%
  mutate(rank = 1:n())
...
levels <- tibble("Pays" = as.factor(main_boards[,.N,Pays][order(Pays)]$Pays)) %>%
  filter(Pays != "OTHER") %>%
  rbind("OTHER") %>%
  mutate(rank = 1:n())
...
journal_name <- tribble(
  ...
  "4713", "The Economic Journal", "dashed", "#a0a0a0",
...
econ_journals <- readRDS(here(data_path, "macro_AA","OST_generic_data", "revues.RDS")) %>% filter(Code_Discipline==119)
...
if(file.exists(here(eer_data,"2_Raw_Networks_and_Alluv", "eer_importance.RDS"))==FALSE){
  ...
  ID_art_journals <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_art.parquet"),
                                         as_data_frame = FALSE) %>% 
    filter(Code_Revue %in% liste_code_revue,
           ItemID_Ref != 0) %>% 
    collect()
  citations_journals <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_ref.parquet"),
                                            as_data_frame = FALSE) %>%  
    filter(ItemID_Ref %in% ID_art_journals$ItemID_Ref,
           Annee >= 1969) %>% # Reviewer suggestion, to no overweight older journals
    collect()
  info_citing <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_art.parquet"),
                                     as.data.frame = FALSE) %>%  
    filter(ID_Art %in% citations_journals$ID_Art) %>% 
    collect()
  ...
  n_refs <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_ref.parquet")) %>%  
    left_join(ID_Art_years) %>% 
    .[ItemID_Ref!=0 & Annee >= 1969,.N,.(Annee_Bibliographique)]
...
if(file.exists(here(eer_data,"2_Raw_Networks_and_Alluv", "eer_importance_econ.RDS"))==FALSE)
{
  ...
  ID_Art_years <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_art.parquet")) %>%  .[,.N,.(ID_Art,Annee_Bibliographique)]
  n_refs <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_ref.parquet")) %>%  left_join(ID_Art_years) %>% .[ItemID_Ref!=0 & ID_Art %in% ID_art_econ$ID_Art & Annee >= 1969,.N,.(Annee_Bibliographique) ]
...
if(file.exists(here(eer_data,"2_Raw_Networks_and_Alluv", "eer_importance_macro.RDS"))==FALSE)
{
  ...
  n_refs <- arrow::read_parquet(here(data_path, "macro_AA","OST_generic_data", "all_ref.parquet")) %>%  
    left_join(ID_Art_years) %>% 
    .[ItemID_Ref!=0 & ID_Art %in% ID_art_macro$ID_Art & Annee >= 1969,.N,.(Annee_Bibliographique)]
  
  citations_journals_macro <- citations_journals[ID_Art %in% ID_art_macro$ID_Art]
...
ggplot(count[Annee_Bibliographique>1970 & Annee_Bibliographique<=2002 & Code_Revue!=12488 & Code_Revue!=14501 & Code_Revue!=16273], aes(x=Annee_Bibliographique , y=share, color=color_bw, linetype = line_type)) +
  geom_smooth(method="auto", se=FALSE, fullrange=FALSE, level=0.95, span = 0.75)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
  scale_y_continuous(NULL, labels = scales::percent_format(scale=1)) +
  scale_x_continuous(breaks=c(1970, 1980, 1990, 2000)) +
  ...
  scale_color_identity() +
  guides(linetype = FALSE, color = FALSE) +
  ggrepel::geom_label_repel(aes(label = Journal_label), data = count[Annee_Bibliographique==2002 & Code_Revue!=12488 & Code_Revue!=14501 & Code_Revue!=16273], nudge_x = 3, segment.color = NA, size=3.5, box.padding = 0.01) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  labs(title = NULL,
       x = NULL) 
ggsave(here(eer_data,"pictures","Graphs","EER_importance_macro_minimal_bw.png"), width=40, height=20, units = "cm", scaling = 2.3)
```
...
ggplot(g1_citation, aes(x=Annee_Bibliographique , y=share, color=color)) +
  ...
  ggrepel::geom_label_repel(aes(label = label_special), data = g1_citation[Annee_Bibliographique==2000, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3, box.padding = 0.01) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  ...
  labs(title = NULL,
       color = NULL,
       x = NULL) 
ggsave(here(eer_data,"pictures","Graphs","g1_citations.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
ggplot(g1_citation, aes(x=Annee_Bibliographique , y=share, color=color_bw, linetype = line_type)) +
  ...
  labs(title = NULL,
       color = NULL,
       x = NULL) 
ggsave(here(eer_data,"pictures","Graphs","g1_citations_bw.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
g1_citation <- g1_citation %>% rename(US_cite_obs = "USA Only") %>% as.data.table()
...
prop_th <- prop_th %>% rename(US_cite_th = "USA Only") %>% as.data.table()
...
ggplot(obs_th_comparison[!is.na(y)], aes(x=Annee_Bibliographique , y=-1*y, color=color, group=color)) +
  ...
  theme_minimal() +
  theme(axis.title = element_text(size = 8), plot.background = element_rect(fill = 'white', colour = NA)) +
  ggrepel::geom_label_repel(aes(label = label_special), data = obs_th_comparison[Annee_Bibliographique==1999, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3, box.padding = 0.01) +
  coord_cartesian(xlim = c(NA, 2003), ylim = c(-0.1,NA)) +
  scale_colour_identity() +
  ggplot2::annotate("text", x = 1989, y = 0.33, 
                    ...
                    size=2.8,color="Darkgrey", parse = TRUE)
ggsave(here(eer_data,"pictures","Graphs","g1_over_citations.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
ggplot(obs_th_comparison[!is.na(y)], aes(x=Annee_Bibliographique , y=-1*y, color=color_bw, group=color, linetype = line_type)) +
  ...
  ggplot2::annotate("text", x = 1989, y = 0.33, 
                    label=TeX(r'($LogRatio=-1 \times\log(\frac{Share\,of\,Citations\,from\,American\,Articles\,to\,Specific\,References}{Overall\,Share\,of\,Citations\,from\,American\,Articles})$)'), 
                    size=2.8,color="Darkgrey", parse = TRUE)
ggsave(here(eer_data,"pictures","Graphs","g1_over_citations_bw.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
ggplot(g2_citation[Annee_Bibliographique<=1995], aes(x=Annee_Bibliographique , y=share, color=color)) +
  ...
  scale_color_discrete(guide = "none") +
  ggrepel::geom_label_repel(aes(label = label_special), data = g2_citation[Annee_Bibliographique==1995, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3, box.padding = 0.01) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = 'white', colour = NA)) +
  scale_colour_identity() +
  labs(title = NULL,
       color = NULL,
       x = NULL) 
ggsave(here(eer_data,"pictures","Graphs","g2_citations.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
ggplot(g2_citation[Annee_Bibliographique<=1995], aes(x=Annee_Bibliographique, y=share, color=color_bw, linetype = line_type)) +
  ...
  labs(title = NULL,
       color = NULL,
       x = NULL) 
ggsave(here(eer_data,"pictures","Graphs","g2_citations_bw.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
g2_citation <- g2_citation %>% rename(US_cite_obs = "USA Only") %>% as.data.table()
...
ggplot(obs_th_comparison[!is.na(y) & Annee_Bibliographique<=1992], aes(x=Annee_Bibliographique , y=-1*y, color=color, group=color)) +
  ...
  theme_minimal() +
  theme(axis.title = element_text(size = 8), plot.background = element_rect(fill = 'white', colour = NA)) +
  ggrepel::geom_label_repel(aes(label = label_special), data = obs_th_comparison[Annee_Bibliographique==1992, head(.SD, 1),label_special], nudge_x = 1, segment.color = NA, size=3, box.padding = 0.01) +
  coord_cartesian(xlim = c(NA, 1995)) +
  scale_colour_identity()+
  ggplot2::annotate("text", x = 1982.5, y = 0.18, 
                    label=TeX(r'($LogRatio=-1 \times\log(\frac{Share\,of\,Citations\,from\,American\,Articles\,to\,Specific\,References}{Overall\,Share\,of\,Citations\,from\,American\,Articles})$)'), 
                    size=2.8,color="Darkgrey", parse = TRUE)
ggsave(here(eer_data,"pictures","Graphs","g2_over_citations.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
ggplot(obs_th_comparison[!is.na(y) & Annee_Bibliographique<=1992], aes(x=Annee_Bibliographique , y=-1*y, color=color_bw, linetype = line_type)) +
  ...
  ggplot2::annotate("text", x = 1982.5, y = 0.18, 
                    label=TeX(r'($LogRatio=-1 \times\log(\frac{Share\,of\,Citations\,from\,American\,Articles\,to\,Specific\,References}{Overall\,Share\,of\,Citations\,from\,American\,Articles})$)'), 
                    size=2.8,color="Darkgrey", parse = TRUE)
ggsave(here(eer_data,"pictures","Graphs","g2_over_citations_bw.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
The following plot is a way to represent these networks and clusters as an alluvial that tracks how nodes that exists across multiple overlapping windows change or stay in the same communities (flows of the alluvial). In addition to individual clusters detected using previous methods, we also colour-coded clusters that have strong relationships as visualised by the flows with similar, yet slightly different colours:
...
for (com in com_to_inspect) {
  ...
  share_europeans_authors_sd <- alluv_dt_as_nodes[Window>=min(alluv_com$Window) & Window<=max(alluv_com$Window)] %>% 
    ...
    mutate(freq = n / sum(n)) %>% as.data.table() %>% 
    .[EU_US_collab=="Europe Only"] 
  share_europeans_authors_sd_save <- copy(share_europeans_authors_sd)
  ...
  share_europeans_authors_cluster_sd <- alluv_com %>%
    ...
    mutate(freq = n / sum(n)) %>% as.data.table() %>% 
    .[EU_US_collab=="Europe Only"]
  share_europeans_authors_cluster_sd <- merge(share_europeans_authors_cluster_sd, share_europeans_authors_sd_save[,.(Window, EU_US_collab)], by = c("Window","EU_US_collab") , all.x = TRUE, all.y = TRUE)
  share_europeans_authors_cluster_sd[is.na(n), n:=0]
  share_europeans_authors_cluster_sd[is.na(freq), freq:=0]
  ...
  share_US_authors_sd_save <- copy(share_US_authors_sd)
  ...
  share_US_authors_cluster_sd <- alluv_com %>%
    ...
    mutate(freq = n / sum(n)) %>% as.data.table() %>% 
    .[EU_US_collab=="USA Only"]
  share_US_authors_cluster_sd <- merge(share_US_authors_cluster_sd, share_US_authors_sd_save[,.(Window, EU_US_collab)], by = c("Window","EU_US_collab") , all.x = TRUE, all.y = TRUE)
  share_US_authors_cluster_sd[is.na(n), n:=0]
  share_US_authors_cluster_sd[is.na(freq), freq:=0]
  share_US_authors_cluster_sd_mean <- share_US_authors_cluster_sd[,mean(freq)]
  share_US_authors_cluster_sd_n <- share_US_authors_cluster_sd[,.N]
  ...
  share_EER_articles_sd <- alluv_dt_as_nodes[Window>=min(alluv_com$Window) & Window<=max(alluv_com$Window)] %>% 
    ...
    mutate(freq = n / sum(n)) %>% as.data.table() %>% 
    .[Revue =="EUROPEAN ECONOMIC REVIEW"]
  share_EER_articles_sd_save <- copy(share_EER_articles_sd)
  ...
  share_EER_articles_cluster_sd <- alluv_com %>%
    ...
    mutate(freq = n / sum(n)) %>% as.data.table() %>% 
    .[Revue=="EUROPEAN ECONOMIC REVIEW"]
  share_EER_articles_cluster_sd <- merge(share_EER_articles_cluster_sd, share_EER_articles_sd_save[,.(Window, Revue)], by = c("Window","Revue"), all.x = TRUE, all.y = TRUE)
  share_EER_articles_cluster_sd[is.na(n), n:=0]
  share_EER_articles_cluster_sd[is.na(freq), freq:=0]
  ...
  europeans_authors_diff_odds <- log10((share_europeans_authors_cluster/share_US_authors_cluster)/(share_europeans_authors/share_US_authors))
  europeans_authors_diff_odds_windows <- log10((share_europeans_authors_cluster_sd_mean/share_US_authors_cluster_sd_mean)/(share_europeans_authors_sd_mean/share_US_authors_sd_mean))
...
plot_odds_bw_windows <- ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff_odds_windows, y = EER_articles_diff_odds_windows)) +
  ...
  theme_classic(base_size = 9) +
  guides(size = "none") + theme(legend.position = "bottom")
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_odds_windows_bw.png"), width=30, height=20, units = "cm", scaling = 1.8)
...
plot_odds_bw_windows <- ggplot(diff_stat_euro_com, aes(x = europeans_authors_diff_odds_windows, y = EER_articles_diff_odds_windows)) +
  ...
  theme_classic(base_size = 9) +
  guides(size = "none") + theme(legend.position = "bottom")
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_odds_windows_bw_wide.png"), width=40, height=20, units = "cm", scaling = 2.3)
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, error=FALSE, results=FALSE, echo=FALSE}
...
for (windows_loop in alluv_dt_as_nodes[order(Window),.N,Window]$Window){
  ...
  result$adj.stand.resid.
  eer_resid <- result$adj.stand.resid. %>% as.data.table
  ...
  result$adj.stand.resid.
  collab_resid <- result$adj.stand.resid. %>% as.data.table
  ...
  plot_resid <- merge(eer_resid[variable %in% com_to_inspect, .SD[2], variable], collab_resid[variable %in% com_to_inspect, .SD[2], variable], by = "variable", all.x = TRUE)
...
plot_resid_big_com <- ggplot(plot_resid, aes(x=1*mean.y, y=1*mean.x, label = Label_com.x)) + 
  ...
  labs(title = NULL,
       color = NULL,
       x = "Mean residuals of European authors: less European (left) vs. more European (right)",
       y = "Mean residuals of EER publications: less European (down) vs. more European (up)") + 
  theme_classic(base_size = 9) +
  guides(size = "none") + 
  theme(legend.position = "bottom")
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, error=FALSE, results=TRUE}
...
noodle_pub[,share_pub_cluster:=n_collab_cluster/n_cluster_window]
noodle_pub[,share_pub_window:=n_collab_window/n_window]

noodle_pub <- noodle_pub[Revue =="EUROPEAN ECONOMIC REVIEW", head(.SD,1), .(Window, Label_com)]
noodle_pub[,log_share:=log10(share_pub_cluster/share_pub_window)]

##################### mean of log for each time windows, however impossible because  you have some time windows with 0 observations for some variables (so you divide by 0)
noodle_collab <- copy(alluv_dt_as_nodes)
...
noodle_collab <- noodle_collab[EU_US_collab=="Europe Only" | EU_US_collab=="USA Only", head(.SD,1), .(Window, Label_com, EU_US_collab)]
noodle_collab <- noodle_collab[,.(new_Id_com, Label_com, share_collab_cluster, share_collab_window, Window, EU_US_collab)]
noodle_collab_cluster <- dcast(noodle_collab, new_Id_com + Window ~ EU_US_collab, value.var="share_collab_cluster") %>% as.data.table()
noodle_collab_cluster[is.na(`Europe Only`), `Europe Only`:=0]
noodle_collab_cluster[is.na(`USA Only`), `USA Only`:=0]
setnames(noodle_collab_cluster, "Europe Only", "cluster_europe_only")
setnames(noodle_collab_cluster, "USA Only", "cluster_usa_only")
noodle_collab_window<- dcast(noodle_collab, new_Id_com + Window ~ EU_US_collab, value.var="share_collab_window") %>% as.data.table()
noodle_collab_window[is.na(`Europe Only`), `Europe Only`:=0]
noodle_collab_window[is.na(`USA Only`), `USA Only`:=0]
setnames(noodle_collab_window, "Europe Only", "window_europe_only")
setnames(noodle_collab_window, "USA Only", "window_usa_only")
noodle_collab <- merge(noodle_collab_cluster, noodle_collab_window, by = c("new_Id_com", "Window"), all.x = TRUE)  

# noodle_collab_cluster[new_Id_com=="NTSLryx5"]

mean_log <- merge(noodle_collab[new_Id_com %in% com_to_inspect], noodle_pub[new_Id_com %in% com_to_inspect,.(new_Id_com, Label_com, share_pub_cluster, share_pub_window, Window)], by = c("new_Id_com", "Window"), all.x = TRUE, all.y = TRUE) %>% as.data.table()
mean_log[, log_on_mean_authors:=log10((cluster_europe_only/cluster_usa_only)/(window_europe_only/window_usa_only))]
mean_log[, log_on_mean_pub:=log10((share_pub_cluster/(1-share_pub_window))/(share_pub_window/(1-share_pub_window)))]

log_ratio_authors_window <- ggplot(mean_log[new_Id_com %in% com_to_inspect], aes(x=as.numeric(Window), y=log_on_mean_authors, group=Label_com)) +
  geom_point() +
  ...
  labs(title = NULL,
       color = NULL,
       y = "Log of ratios: US only (down) vs. European only (top)",
       x = "Time window")
ggsave(here(eer_data,"pictures","Graphs","log_ratio_authors_window.png"), width=30, height=20, units = "cm", scaling = 0.7)

log_ratio_eer_window <- ggplot(mean_log[new_Id_com %in% com_to_inspect], aes(x=as.numeric(Window), y=log_on_mean_pub, group=Label_com)) +
  geom_point() +
  geom_smooth(se = FALSE, color = "black") +
  geom_hline(yintercept = 0, size = 1, alpha = 0.4) +
  theme_classic() +
  ...
  labs(title = NULL,
       color = NULL,
       y = "Log of Ratios: Top 5 (down) vs. EER (top)",
       x = "Time window")
ggsave(here(eer_data,"pictures","Graphs","log_ratio_eer_window.png"), width=30, height=20, units = "cm", scaling = 0.7)


mean_log[!is.nan(log_on_mean_authors),mean_log_aut:=mean(log_on_mean_authors), new_Id_com]
mean_log[!is.nan(log_on_mean_authors),mean_log_pub:=mean(log_on_mean_pub), new_Id_com]
mean_log <- mean_log[, head(.SD,1), .(new_Id_com)]
mean_log <- merge(mean_log, new_Id_com_size, by="new_Id_com", all.x = TRUE) # size dot

plot_mean_log <- ggplot(mean_log[new_Id_com %in% com_to_inspect], aes(x = mean_log_aut, y = mean_log_pub)) +
  geom_vline(aes(xintercept = 0), size = 1, alpha = 0.3) +
  geom_hline(aes(yintercept = 0), size = 1, alpha = 0.3) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", color = "gray", alpha = 0.5) +
  geom_point(aes(size = new_Id_com_size), alpha = 0.8, color = "#808080") +
  ...
  theme_classic(base_size = 9) +
  guides(size = "none") + theme(legend.position = "bottom")
ggsave(here(eer_data,"pictures","Graphs","Communities_europeanisation_mean_of_log_ratio.png"), width=30, height=20, units = "cm", scaling = 1.5)
...
for (com in com_to_inspect) {
  ...
  cat("Its most distinctive words are: ",tf_idf_results$list_words[Leiden1 %like% paste0(alt_com_name)]$term)
...
All the documents of our corpus (a document is an article's title and abstract) have a 'rate of belonging' with each topic (the *gamma* value) that represents the extent to which a topic is present in the document. We are able, for each topic, to compute the difference in the means of *gamma* values for *(i)* articles published in the EER and articles published in the Top 5 and *(ii)* articles written by US-based authors and those written by European-based authors. The two differences are plotted in a two-dimensional graph to observe which topics are over-represented in the EER and more frequently mentioned by Europe-based authors.
...
Rather than averaging on all documents, we can take the documents the most associated to each topic. That is what we did in the article by keeping only the articles with a *gamma* value superior to 0.1 for each topic. We then assess the over/under representation of EER articles for each topic, by calculating the log odds ratio in comparison to top 5 articles:
...
with $N_{\text{EER in topic }i}$ the number of EER articles in the topic and $N_{\text{topic }i}$ the total number of articles in the topic. We perform the same calculation for articles written exclusively by Europe-based authors or US-based authors. 
